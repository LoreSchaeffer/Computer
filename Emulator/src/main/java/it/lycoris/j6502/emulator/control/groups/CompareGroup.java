...
public class CompareGroup implements InstructionGroup {
    ...
    public void install(Map<Integer, OpcodeMetadata> registry) {
        ...
        registry.put(0xC5, new OpcodeMetadata("CMP $zp", cpu -> cmp(cpu, cpu.getAccumulator(), cpu.readSystemBus(cpu.addrZeroPage()))));
        registry.put(0xCD, new OpcodeMetadata("CMP $abs", cpu -> cmp(cpu, cpu.getAccumulator(), cpu.readSystemBus(cpu.addrAbsolute()))));

        // --- CPX (Compare X) ---
        registry.put(0xE0, new OpcodeMetadata("CPX #", cpu -> cmp(cpu, cpu.snapshot().x(), cpu.fetchOperand())));
        registry.put(0xE4, new OpcodeMetadata("CPX $zp", cpu -> cmp(cpu, cpu.snapshot().x(), cpu.readSystemBus(cpu.addrZeroPage()))));
        registry.put(0xEC, new OpcodeMetadata("CPX $abs", cpu -> cmp(cpu, cpu.snapshot().x(), cpu.readSystemBus(cpu.addrAbsolute()))));

        // --- CPY (Compare Y) ---
        registry.put(0xC0, new OpcodeMetadata("CPY #", cpu -> cmp(cpu, cpu.snapshot().y(), cpu.fetchOperand())));
        registry.put(0xC4, new OpcodeMetadata("CPY $zp", cpu -> cmp(cpu, cpu.snapshot().y(), cpu.readSystemBus(cpu.addrZeroPage()))));
        registry.put(0xCC, new OpcodeMetadata("CPY $abs", cpu -> cmp(cpu, cpu.snapshot().y(), cpu.readSystemBus(cpu.addrAbsolute()))));
    }
